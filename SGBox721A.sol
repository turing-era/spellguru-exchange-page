...
/**@title SGBox Contract
 * @author Turingera
 */
contract SGBox721A is ERC721A, ERC2981, Ownable, EIP712, ReentrancyGuard {
    struct TimeRange {
    ...
    ) Ownable(msg.sender) ERC721A(name, symbol) EIP712(name, VERSION) {
        _baseUri = baseUri;
    ...
    function purchase(
        ...
        require(
            quantity != 0 && quantity + _numberMinted(to) <= purchaseLimit,
            "Invalid quantity"
        );
        _handlePurchase(
            quantity,
